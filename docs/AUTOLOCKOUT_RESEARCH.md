...
2. ~~**Zone capacity** — 200 slots may be tight for heavy commuters; consider dynamic expansion or prioritized eviction~~ **Done** — capacity raised to 500 with LRU eviction of lowest-confidence learned zones
...
1. ~~**Monolithic file size** — 1893 lines is unmaintainable.~~ **Improved** — reduced to 1888 lines via extraction of constants/utils module, Ka Warning Modal, and Zone Editor Modal components. Next largest page (colors) is 1351 lines.
...
7. ~~Simplify zone editor: pre-fill with current GPS location, support "create from observation"~~ **Done**
8. ~~Add per-band auto-learn toggles (K/Ka/X) — currently only Ka has a toggle~~ **Done** (`dd296d8`)
...
11. ~~Import/export UI improvements~~ **Done** (`6eb9968`)
12. ~~Component extraction: break monolithic file into Svelte components~~ **Done** (`e266396`)
...
Tracking completed improvements from the Phase 2 plan and enhancement areas.

| # | Item | Status | Commit | Notes |
|---|------|--------|--------|-------|
| 1 | GPS quality gate settings in UI | **Done** | Prior commits | HDOP threshold, min satellites, min speed exposed |
| 2 | Per-band auto-learn toggles (K/X/Ka) | **Done** | `dd296d8` | Individual K, X toggles + Ka with warning modal |
| 3 | Visual differentiation of dangerous settings | **Done** | Prior commits | Ka toggle-warning, confirmation modal, safety gate |
| 4 | Quick-create zone from observation | **Done** | Prior commits | "Lock Out" button on signal observations |
| 5 | Import/export improvements | **Done** | `6eb9968` | Merge/replace modes, zone counts, Clear All button |
| 6 | Zone capacity beyond 200 | **Done** | `34ead94` | Raised to 500 + learned-zone eviction policy |
| 7 | Component extraction | **Done** | `e266396` | Utils module, Ka modal, zone editor modal extracted |

### Remaining Items

| Priority | Item | Status | Notes |
|----------|------|--------|-------|
| Tier 3 | Map visualization (Leaflet/OSM) | Not started | Biggest single gap vs competitors |
| Tier 2 | Responsive tables (card on mobile) | Not started | Active zones table requires h-scroll |
| Tier 2 | Contextual help tooltips | Not started | Every setting needs plain-English description |
| Tier 2 | Progressive disclosure | Not started | Collapse advanced settings behind expandable sections |
| Tier 3 | GPX/KML import | Not started | Import zones from standard map formats |
| Tier 3 | Analytics dashboard | Not started | Hit rates, false-positive analysis |
