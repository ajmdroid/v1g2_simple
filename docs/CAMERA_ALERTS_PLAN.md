...
> Note: This draft captures pre-ALPR-only design stages. Current runtime/data
> path is ALPR-only (`alpr.bin`).
...
- Loader task, immutable index build/swap, and ALPR matching are implemented under `src/modules/camera/`.
- Camera APIs are registered in `src/wifi_routes.cpp` with service logic in `src/modules/camera/camera_api_service.*`: `/api/cameras/status`, `/api/cameras/events`, and `/api/cameras/catalog`.
- Camera UI page is active at `interface/src/routes/cameras/+page.svelte` with runtime/status/event/catalog views.
- Persistent `cameraEnabled` setting is active and controls camera runtime/index loading.
- Live matching remains gated by GPS eligibility (enabled + valid fix/course).
...
4. Active GPS APIs are registered in `src/wifi_routes.cpp` with service logic in `src/modules/gps/gps_api_service.*`:
...
6. Active camera APIs are registered in `src/wifi_routes.cpp` with service logic in `src/modules/camera/camera_api_service.*`:
...
1. M2+ default: load ALPR dataset (`alpr.bin`) for runtime matching.
2. Legacy speed/redlight dataset staging is no longer part of active runtime.
...
5. M2 operational mode:
   - ALPR dataset (`alpr.bin`) is default runtime input,
   - span-budget and perf checks still gate acceptance/load.
6. Load path:
   - decode VCAM records,
   - compute `cellKey`,
   - sort by `cellKey`,
...
3. Dataset catalog table (`alpr`) is implemented.
...
3. Enable ALPR query path with raw scan cap and
...
|---------|--------------|----------------------------------|----------------|
| ALPR dataset (`alpr`) | 38,623 | ~452.6 KiB | 81 |

Implications:

1. Current data does **not** show 3x3 candidate explosion (`max=81`), so
   `<=128` raw scan cap is currently sufficient.
2. ALPR span metadata is too large for current 64 KiB span budget and
...
| F2 No index detail | Gap addressed for M2 | **Verified with repository data + sizing math** | v0.6/v0.7 lock scan-cap ordering and span budget checks for ALPR runtime loading. |
...
| F8 ALPR dominance | Gap addressed in plan | **Verified data** | Data skew is verified; v0.5+ keeps ALPR as the active runtime dataset with explicit gating. |
...
7. **Dataset staging:** ALPR is default runtime path and remains gated by
   span/memory/perf checks.
...
3. Dataset catalog observability:
   ...
   - UI shows ALPR `present/valid/count/bytes`.
4. Camera runtime control:
   - Persistent `cameraEnabled` setting implemented.
   - Effective runtime state is enforced as `cameraEnabled`.
   - Live match eligibility still requires GPS enabled + valid fix/course.
...
2. If GPS is disabled, camera index can still load when `cameraEnabled=true`,
   but live matching remains inactive.
