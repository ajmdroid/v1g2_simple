...
_DEFAULT_INVENTORY="$ROOT_DIR/test/device/board_inventory.json"
_LOCAL_INVENTORY="${_DEFAULT_INVENTORY%.json}.local.json"
if [[ -z "${HARDWARE_TEST_INVENTORY:-}" && -f "$_LOCAL_INVENTORY" ]]; then
  _DEFAULT_INVENTORY="$_LOCAL_INVENTORY"
fi
INVENTORY_PATH="${HARDWARE_TEST_INVENTORY:-$_DEFAULT_INVENTORY}"
