...
void DisplayPreviewModule::update() {
    ...
    if (previewMode == PreviewMode::ALERT) {
        ...
        if (previewStep < STEP_COUNT && elapsed >= STEPS[previewStep].offsetMs) {
            ...
            if (step.kind == PreviewStepKind::RADAR) {
                ...
                payload.type = step.cameraType;
                payload.distanceCm = step.cameraDistanceCm;

                DisplayState previewState{};
                previewState.arrows = DIR_FRONT;
                display->updateCameraAlert(payload, previewState);
            }
            previewStep++;
        }
    }
