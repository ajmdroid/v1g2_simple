...
void DisplayPreviewModule::update() {
    ...
    if (previewStep < STEP_COUNT && elapsed >= STEPS[previewStep].offsetMs) {
        ...
        // Check for special OBD idle preview step
        if (step.showObdIdle) {
            // Show resting screen with OBD idle data preview
            display->showResting(true);  // Force redraw to get base frame
            display->drawIdleObdDataPreview();  // Draw mock OBD data
            display->flush();
            previewStep++;
        } else {
