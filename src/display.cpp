...
void V1Display::showDeleteLogsScreen() {
#if defined(DISPLAY_USE_ARDUINO_GFX)
    // Clear screen to dark background
    tft->fillScreen(0x0000);
    
    // Layout: 640x172 landscape - title + two buttons
    
    // Title
    tft->setTextColor(0xFFFF);  // White
    tft->setTextSize(2);
    tft->setCursor((SCREEN_WIDTH - 200) / 2, 20);
    tft->print("DELETE DEBUG LOGS?");
    
    // Warning text
    tft->setTextSize(1);
    tft->setTextColor(0xFD20);  // Orange
    tft->setCursor((SCREEN_WIDTH - 300) / 2, 55);
    tft->print("This will permanently erase /debug.log");
    
    // Buttons - Cancel on left, Delete on right
    const int buttonWidth = 180;
    const int buttonHeight = 50;
    const int buttonY = 95;
    const int buttonSpacing = 80;
    const int cancelX = (SCREEN_WIDTH / 2) - buttonWidth - (buttonSpacing / 2);
    const int deleteX = (SCREEN_WIDTH / 2) + (buttonSpacing / 2);
    
    // Cancel button (gray background)
    tft->fillRoundRect(cancelX, buttonY, buttonWidth, buttonHeight, 8, 0x4208);
    tft->drawRoundRect(cancelX, buttonY, buttonWidth, buttonHeight, 8, 0x8410);
    tft->setTextColor(0xFFFF);
    tft->setTextSize(2);
    tft->setCursor(cancelX + (buttonWidth - 72) / 2, buttonY + (buttonHeight - 14) / 2);
    tft->print("CANCEL");
    
    // Delete button (red background)
    tft->fillRoundRect(deleteX, buttonY, buttonWidth, buttonHeight, 8, 0xF800);
    tft->drawRoundRect(deleteX, buttonY, buttonWidth, buttonHeight, 8, 0xFBE0);
    tft->setTextColor(0xFFFF);
    tft->setTextSize(2);
    tft->setCursor(deleteX + (buttonWidth - 72) / 2, buttonY + (buttonHeight - 14) / 2);
    tft->print("DELETE");
...
// Returns which button was touched: 0=cancel, 1=delete, -1=none
// Touch coordinates are in display space after rotation
int V1Display::getDeleteLogsButtonFromTouch(int16_t touchX, int16_t touchY) {
    const int buttonWidth = 180;
    const int buttonHeight = 50;
    const int buttonY = 95;
    const int buttonSpacing = 80;
