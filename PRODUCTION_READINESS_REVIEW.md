...
The project is production-ready. After secondary verification and targeted follow-up fixes, the previously validated P0 items and the actionable P1/P2 cleanups from this review have been addressed. The remaining notes below are advisory or contextual, not ship blockers.
...
### 1. ~~Loop Duration SLO Metric Does Not Measure What It Claims~~ — RESOLVED
...
> **Resolution status**: Fixed. Full-loop duration is now recorded at loop tail after finalize/yield work, so the authoritative `loopMax_us` metric covers the entire loop instead of a mid-loop slice.
...
### 2. ~~Settings Mode Early Return Skips BLE Drain and Finalize-Phase Work~~ — RESOLVED
...
> **Resolution status**: Fixed. Settings-mode loops now take a reduced finalize path that preserves maintenance, forces BLE drain, performs the required yield, and records end-of-loop timing without re-entering the full display/WiFi phases.
...
### 5. ~~Battery Shutdown Path Blocks on I2C~~ — RESOLVED
...
> **Resolution status**: Fixed. The latch-drop readback path now uses an explicit bounded `TwoWire::setTimeOut()` scope with timeout restoration, plus the shared I2C mutex guard, so readback cannot block the deep-sleep fallback indefinitely.
...
### 6. ~~Legacy Dirty Region Code Is Dead but Still Executing~~ — RESOLVED
...
> **Resolution status**: Fixed. The retired frequency/card dirty-region fields and writes were removed from the hot path, leaving only the currently used full-canvas and explicit-region flush logic.
...
### 7. ~~Unconditional `vTaskDelay(1)` Every Loop~~ — DOCUMENTED AS INTENTIONAL
...
> **Resolution status**: Clarified in code. The unconditional one-tick yield remains, but it is now explicitly documented as intentional scheduler/TWDT behavior rather than an accidental omission.
...
> **Current status**: Already documented correctly. `src/settings.cpp` and `docs/MANUAL.md` explicitly call this XOR obfuscation, not encryption.
...
> **Current status**: Left as an incremental style rule, not a surgical cleanup target.
...
> **Current status**: Retained intentionally. This is an active workaround for a real panel quirk, not dead code.

### 11. ~~`audio_process_amp_timeout()` Called Every Loop~~ — RESOLVED
...
> **Resolution status**: Fixed. The housekeeping check now runs at a bounded 10 Hz cadence, which preserves the same user-visible 3-second timeout behavior while removing pointless per-loop polling.
...
> **Current status**: Advisory only. No code change recommended without a separate OTA/storage decision.
...
Ship it. The previously validated P0 items and the actionable P1/P2 follow-ups called out here are now addressed. The remaining items are advisory, contextual, or deliberately retained behaviors.

The testing infrastructure gives high confidence that these targeted fixes won't introduce regressions. Run `scripts/ci-test.sh` after each fix set, and you're good.
...
|---|---|---|
| P0-1: Loop SLO metric | Incomplete measurement | **Confirmed and fixed** — authoritative loop timing now records at loop tail after finalize work |
| P0-2: Settings early return | Skips BLE drain | **Confirmed and fixed** — settings-mode now takes a reduced finalize path with forced drain and loop timing |
...
| P1-5: Battery I2C blocking | 50ms timeout risk | **Corrected and fixed** — latch-drop readback now uses explicit bounded `TwoWire` timeout handling |
