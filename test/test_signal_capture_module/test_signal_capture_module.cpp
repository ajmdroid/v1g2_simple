...
void test_first_alert_publishes_and_enqueues() {
    ...
    TEST_ASSERT_TRUE(std::isnan(out[0].speedMph));
    TEST_ASSERT_EQUAL_UINT8(static_cast<uint8_t>(SpeedSource::NONE), out[0].speedSourceRaw);
}
...
void test_missing_selected_speed_does_not_fallback_to_raw_gps_speed() {
    PacketParser parser;
    parser.setAlerts({AlertData::create(BAND_K, DIR_FRONT, 3, 5, 24148, true, true)});
    GpsRuntimeStatus gps = makeGps();
    gps.speedMph = 12.0f;
...
int main() {
    ...
    RUN_TEST(test_missing_selected_speed_does_not_fallback_to_raw_gps_speed);
