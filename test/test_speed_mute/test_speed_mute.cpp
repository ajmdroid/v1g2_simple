...
void test_module_begin_and_update() {
    SpeedMuteModule mod;
    mod.begin(true, 25, 3, true, false);
...
void test_module_sync_settings() {
    SpeedMuteModule mod;
    mod.begin(false, 25, 3, true, false);  // disabled

    auto d = mod.update(10.0f, true, 2000);
    TEST_ASSERT_FALSE(d.shouldMute);  // disabled

    mod.syncSettings(true, 25, 3, true, false);  // enable
    d = mod.update(10.0f, true, 4000);
    TEST_ASSERT_TRUE(d.shouldMute);  // now should mute
}

void test_module_band_override_laser() {
    SpeedMuteModule mod;
    mod.begin(true, 25, 3, true, false);  // laser override ON, ka override OFF

    // Band 1 = LASER → overridden
    TEST_ASSERT_TRUE(mod.isBandOverridden(1));
    // Band 2 = KA → not overridden
    TEST_ASSERT_FALSE(mod.isBandOverridden(2));
    // Band 4 = K → not overridden
    TEST_ASSERT_FALSE(mod.isBandOverridden(4));
}

void test_module_band_override_ka() {
    SpeedMuteModule mod;
    mod.begin(true, 25, 3, true, true);  // both overrides ON
...
void test_module_no_overrides() {
    SpeedMuteModule mod;
    mod.begin(true, 25, 3, false, false);  // all overrides OFF

    TEST_ASSERT_FALSE(mod.isBandOverridden(1));
    TEST_ASSERT_FALSE(mod.isBandOverridden(2));
}
...
int main() {
    ...
    RUN_TEST(test_module_band_override_laser);
    RUN_TEST(test_module_band_override_ka);
    RUN_TEST(test_module_no_overrides);
